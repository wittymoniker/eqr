#!/bin/bash
# ==============================================================================
# ADVANCED TENSOR REALITY ENGINE - MASTER VOLUMETRIC EDITION (WINMAC / CROSS-PLATFORM)
...
# ANSI Color formatting safely handled
clear
echo "=============================================================================="
echo "    TENSOR REALITY ENGINE - WINMAC VOLUMETRIC MASTER EDITION                 "
echo "=============================================================================="

# 1. TIMESCALE & TIME OFFSET CONFIGURATION
echo -e "\n[1/7] Select Base Time Scale Unit:"
echo "  [1] Gigasecond  (1 Gs  = 10^9 s)"
echo "  [2] Megasecond  (1 Ms  = 10^6 s)"
echo "  [3] Second      (1 s   = 1.0 s)"
echo "  [4] Millisecond (1 ms  = 10^-3 s)"
echo "  [5] Microsecond (1 us  = 10^-6 s) [DEFAULT]"
...
echo -e "\n[2/7] Centimeter-Based Spatial Dimension Scale & Volumetric Offsets:"
...
case "$SPATIAL_CHOICE" in
    ...
    5) SPATIAL_BASE_VAL="1e-4"; SPATIAL_LABEL="um" ;;
    6) SPATIAL_BASE_VAL="1e-7"; SPATIAL_LABEL="nm" ;;
    7) SPATIAL_BASE_VAL="1e-10"; SPATIAL_LABEL="pm" ;;
    *) SPATIAL_BASE_VAL="1.0"; SPATIAL_LABEL="cm" ;;
esac

# Explicit prompt requested for exact field number of meters for the scaling and index of the equation graph
...
echo -e "\n[3/7] 3D Volumetric Scheme & Optimization Techniques:"
...
echo -e "\n[4/7] Target Labeling Heuristic Profiles:"
...
echo -e "\n[4.1] Acoustic Audio Sonification Profile (Ears Option):"
...
echo -e "\n[5/7] Procedural Effects & Camera Constraints Configuration:"
...
export OUTPUT_FILE="$SCRIPT_DIR/tensor_winmac_volumetric_render_$TIMESTAMP.mp4"
export AUDIO_FILE="$SCRIPT_DIR/tensor_winmac_audio_$TIMESTAMP.wav"

echo -e "\n[+] WinMac Volumetric Master Parameters Locked:"
echo "    - Metric Scale       : ${SPATIAL_LABEL}"
echo "    - Exact Field Meters : ${EXACT_METERS_SCALE} m"
echo "    - Volumetric Scheme  : ${VOL_SCHEME} (Res: ${VOL_RES}, Threshold: ${VOL_THRESHOLD})"
echo "    - Time Offset        : ${TIME_OFFSET} units"
echo "    - XYZ Offsets        : X=${OFFSET_X}, Y=${OFFSET_Y}, Z=${OFFSET_Z}"
echo "    - Output Target      : $OUTPUT_FILE\n"

# ==============================================================================
# 6. PYTHON WINMAC VOLUMETRIC 3D ENGINE WRITER
# ==============================================================================
cat << 'EOF' > /tmp/tensor_winmac_engine.py
import numpy as np
import matplotlib
matplotlib.use('Agg')
...
def signal_handler(sig, frame):
    print("\n[Python WinMac] Interrupted cleanly by user. Saving state...")
    sys.exit(0)

signal.signal(signal.SIGINT, signal_handler)

try:
    print("[Python WinMac] Initializing Cross-Platform Volumetric Tensor Reality Engine...")
    c_base = 134964356.0
    ...
    output_file = os.environ.get("OUTPUT_FILE", "tensor_winmac_render.mp4")
    ...
    def get_available_encoder():
        try:
            ...
            elif 'h264_videotoolbox' in res.stdout: return 'h264_videotoolbox' # Optimized for macOS Apple Silicon
            elif 'h264_nvenc' in res.stdout: return 'h264_nvenc'             # Optimized for Windows NVIDIA
        except Exception:
            pass
        return 'mpeg4'

    chosen_encoder = get_available_encoder()
    print(f"[Python WinMac] Selected FFmpeg video encoder: {chosen_encoder}")
    ...
    for i in range(total_frames):
        ...
        ax.set_title(f"WINMAC 3D [{vol_scheme.upper}] | Scale: {spatial_label} ({exact_meters}m) | T={t*1e6:.2f}us", color='#00ffcc', fontsize=9, fontweight='bold')
        ...
        if i % log_interval == 0 or i == total_frames - 1:
            print(f"[Python WinMac] Rendered volumetric frame {i+1}/{total_frames} ({(i+1)/total_frames*100:.1f}%)")
    ...
    print("[Python WinMac] Volumetric Master Pipeline Render Completed Successfully.")

except Exception as e:
    print(f"[Python WinMac Error]: {e}")
    traceback.print_exc()
    sys.exit(1)
EOF

python3 /tmp/tensor_winmac_engine.py
rm -f /tmp/tensor_winmac_engine.py

echo "=============================================================================="
echo "       WINMAC VOLUMETRIC PIPELINE RENDER FINISHED SUCCESSFULLY               "
echo "=============================================================================="
echo "Saved file: $OUTPUT_FILE"#!/bin/bash
# ==============================================================================
